        ...
        if self.notifier:
            try:
                await self.notifier.notify(
                    "system_alert", 
                    {"message": msg, "source": "HealthDaemon"}, 
                    severity=severity
                )
            except Exception as e:
    ...
    async def run_forever(self):
         ...
         while True:
             ...
             if status == HeartbeatStatus.ALIVE:
                 self.last_alive = now
                 if self.last_restart and (now - self.last_restart).total_seconds() > RESET_COUNT_AFTER_S:
                      self.restart_count = 0
             ...
             if status == HeartbeatStatus.DEAD:
                 ...
                 if self.last_restart and (now - self.last_restart).total_seconds() < COOLDOWN_S:
                     log.info("Masih dalam masa cooldown restart. Skip.")
                      ...
                      if not success:
                          await self._send_alert("Perintah restart GAGAL dieksekusi oleh sistem.", severity="critical")
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] HealthDaemon: %(message)s")
    daemon = HealthDaemon()
