...
class LLMBudget:
    def __init__(self, limits: LLMBudgetLimits, storage_file: str = "runtime/agent/llm_layer/budget.json"):
        self._limits = limits
    ...
    async def can_call(self, estimated_tokens: int = 500, model: str = 'claude-haiku-4-5-20251001') -> bool:
        ...
        est_cost = (estimated_tokens * pricing['input'] + estimated_tokens * pricing['output']) / 1_000_000
        
        if (self._spent_today + est_cost) > self._limits.daily_usd_limit:
            log.warning(f"Budget USD LLM Habis! Spent: ${self._spent_today:.4f}, Est: ${est_cost:.4f}")
            return False
