...
class LLMRateLimiter:
    ...
    def acquire(self, estimated_tokens: int = 500) -> bool:
        ...
        self._call_bucket.tokens = min(
            self._call_bucket.capacity, self._call_bucket.tokens + add_calls
        )
        ...
        self._token_bucket.tokens = min(
            self._token_bucket.capacity, self._token_bucket.tokens + add_toks
        )
