...
class AdaptationEngine:
    ...
    def evaluate(self, stats: StrategyStats, config: Any) -> list[AdaptationResult]:
        ...
        if stats.win_rate < 0.45 and stats.total_trades >= 10:
            ...
            new_val = min(
                old_val + self.LIMITS["min_signal_confidence"]["step"],
                self.LIMITS["min_signal_confidence"]["max"],
            )
            if new_val != old_val:
                proposals.append(
                    AdaptationResult(
        ...
        if stats.avg_loss_usd > 12.0 and stats.total_trades >= 20:
            ...
            new_val = min(
                old_val + self.LIMITS["sl_atr_multiplier"]["step"],
                self.LIMITS["sl_atr_multiplier"]["max"],
            )
            if new_val != old_val:
                proposals.append(
                    AdaptationResult(
    ...
    async def apply(self, result: AdaptationResult, config: Any, dry_run: bool = False) -> bool:
        ...
        if dry_run:
            log.info(
                f"[DRY-RUN] Akan mengubah {result.parameter_name} dari {result.old_value} ke {result.new_value}"
            )
            return True
