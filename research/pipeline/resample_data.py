...
def resample_ohlcv(
    ...
    if TF_SECONDS.get(target_tf, 0) <= TF_SECONDS.get(source_tf, 0):
        raise ValueError(f"target_tf ({target_tf}) harus lebih besar dari source_tf ({source_tf})")
    ...
    log.info(f"Resampled {source_tf} → {target_tf}: {len(df)} → {len(resampled)} baris")
    return resampled
...
def align_multi_tf(
    ...
    for tf, df in dfs.items():
        filtered = df[(df["timestamp"] >= global_start) & (df["timestamp"] <= global_end)].copy()
