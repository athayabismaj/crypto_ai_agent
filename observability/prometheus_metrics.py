...
class AgentMetrics:
    ...
    def __init__(self):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self._gauges["daily_pnl_usd"] = Gauge(
                "daily_pnl_usd", "PnL hari ini per strategi", ["mode", "strategy"]
            )
            self._gauges["open_positions_total"] = Gauge(
                "open_positions_total", "Jumlah posisi terbuka", ["symbol", "side"]
            )
            self._gauges["win_rate_30d"] = Gauge(
                "win_rate_30d", "Win rate 30 hari terakhir per strategi", ["strategy"]
            )
            self._gauges["circuit_breaker_state"] = Gauge(
                "circuit_breaker_state", "0=normal, 1=warned, 2=halted", ["mode"]
            )
            self._gauges["drawdown_pct"] = Gauge(
                "drawdown_pct", "Drawdown dari peak dalam persen", ["mode"]
            )
            self._gauges["ws_last_message_age_s"] = Gauge(
                "ws_last_message_age_s",
                "Detik sejak pesan WebSocket terakhir",
                ["symbol", "stream"],
            )
            self._gauges["api_weight_used"] = Gauge(
                "api_weight_used", "API weight yang digunakan", ["exchange"]
            )

            self._counters["trade_opened_total"] = Counter(
                "trade_opened_total", "Total trade dibuka", ["symbol", "strategy"]
            )
            self._counters["trade_closed_total"] = Counter(
                "trade_closed_total", "Total trade ditutup", ["symbol", "exit_reason"]
            )
            self._counters["llm_cost_usd_total"] = Counter(
                "llm_cost_usd_total", "Total biaya LLM", ["model"]
            )
            self._counters["signal_generated_total"] = Counter(
                "signal_generated_total", "Total signal dihasilkan", ["symbol", "strategy"]
            )
            self._counters["signal_blocked_total"] = Counter(
                "signal_blocked_total", "Total signal diblokir", ["symbol", "reason"]
            )

            self._histograms["order_latency_ms"] = Histogram(
                "order_latency_ms", "Latency order ke exchange", ["exchange", "endpoint"]
            )
            self._histograms["db_query_duration_ms"] = Histogram(
                "db_query_duration_ms", "Latency query database", ["db", "operation"]
            )
    ...
    def update_all(self, portfolio_state: dict, system_state: dict, mode: str = "live") -> None:
        ...
        try:
            ...
            if "daily_pnl" in portfolio_state:
                self._gauges["daily_pnl_usd"].labels(mode=mode, strategy="all").set(
